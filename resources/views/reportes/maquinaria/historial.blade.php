                        ...
                        <tr class="hover:bg-gray-50">
                                    ...
                                    <div class="grid grid-cols-1 gap-1">
                                        @foreach($equipo as $p)
                                            <div class="text-sm text-gray-800 leading-tight">
                                                      {{ is_array($p) ? ($p['nombre'] ?? '—') : (is_object($p) ? ($p->nombre ?? '—') : $p) }}
                            ...
                            <td class="px-4 py-3 min-w-[320px]">
                                <div class="space-y-2">
                                    ...
                                    data-maquina-id="{{ $r['maquina_id'] }}"
                                    >{{ $r['observaciones'] ?? '' }}</textarea>

                                    <button
                                    type="button"
                                    class="px-3 py-1.5 rounded-lg bg-gray-900 text-white text-xs hover:bg-gray-800 btn-save-obs"
                                    >
                                    Guardar
                                    </button>

                                    <div class="text-xs text-gray-500 save-status"></div>
                                </div>
                            </td>
...
document.addEventListener('click', async (e) => {
  ...
  const maquinaId = textarea.dataset.maquinaId;
  ...
  try {
    const res = await fetch("{{ route('reportes.maquinaria.historial.observaciones.update') }}", {
      method: 'PATCH',
      headers: {
