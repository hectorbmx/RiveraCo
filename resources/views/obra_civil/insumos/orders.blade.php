...
@section('title', 'Ordenes por insumo')
...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <h1 class="text-2xl font-bold text-[#0B265A]">Ordenes de compra del insumo</h1>
            <p class="mt-1 text-sm text-slate-500">{{ $insumo->codigo }} - {{ $insumo->concepto }}</p>
        </div>
    ...
    <section class="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="grid grid-cols-1 gap-4 p-5 md:grid-cols-4">
            <div>
                ...
                <div class="mt-1 font-semibold text-slate-900">{{ number_format((float) $balance['budget_quantity'], 4) }} {{ $insumo->unidad }}</div>
            ...
            <div>
                ...
                <div class="mt-1 font-semibold text-slate-900">{{ number_format((float) $balance['used_quantity'], 4) }} {{ $insumo->unidad }}</div>
            ...
            <div>
                ...
                <div class="mt-1 font-semibold {{ (float) $balance['available_quantity'] < 0 || (float) $balance['available_amount'] < 0 ? 'text-red-700' : 'text-slate-900' }}">
                    {{ number_format((float) $balance['available_quantity'], 4) }} {{ $insumo->unidad }}
                </div>
                <div class="text-xs {{ (float) $balance['available_amount'] < 0 ? 'text-red-600' : 'text-slate-500' }}">${{ number_format((float) $balance['available_amount'], 2) }}</div>
            </div>
            <div>
