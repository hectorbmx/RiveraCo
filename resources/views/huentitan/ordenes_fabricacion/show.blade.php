...
@php
    $estadoClases = [
        ...
        'pendiente_material' => 'bg-red-50 text-red-700 border-red-200',
        'lista_para_apartar' => 'bg-green-50 text-green-700 border-green-200',
        'apartada' => 'bg-amber-50 text-amber-700 border-amber-200',
    ...
    <div class="bg-white border rounded-lg overflow-hidden">
        <div class="grid grid-cols-1 md:grid-cols-8 divide-y md:divide-y-0 md:divide-x divide-gray-200 text-sm">
            ...
            <div class="p-3 min-w-0 bg-gray-50 flex flex-col justify-center gap-1.5">
                ...
                @if($orden->apartada_at)
                    <div class="flex items-center justify-between text-xs border-t pt-1 border-gray-200"><span class="text-gray-500">Apartada:</span><span class="font-semibold text-gray-900">{{ $orden->apartada_at->format('Y-m-d H:i') }}</span></div>
                @endif
                @if($orden->produccion_iniciada_at)
                    <div class="flex items-center justify-between text-xs border-t pt-1 border-gray-200"><span class="text-gray-500">Produccion:</span><span class="font-semibold text-gray-900">{{ $orden->produccion_iniciada_at->format('Y-m-d H:i') }}</span></div>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="bg-white border rounded-lg overflow-hidden">
        <div class="px-4 py-3 border-b bg-gray-50 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            <div>
                ...
                <p class="text-xs text-gray-500">{{ in_array($orden->estado, ['calculada', 'pendiente_material', 'lista_para_apartar', 'apartada'], true) ? 'Formula congelada; disponibilidad revisada contra stock actual.' : 'Snapshot de formula comparado contra stock actual de ' . $almacen->nombre }}</p>
            </div>
            <div class="text-xs text-gray-500">{{ $orden->estado === 'en_produccion' ? 'Insumos descontados del inventario' : 'Sin afectacion de inventario' }}</div>
        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 text-gray-600">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right">Apartado</th>
                ...
                <tbody class="divide-y">
                    @forelse($materiales as $materialDetalle)
                        @php
                            ...
                            $stockActual = in_array($orden->estado, ['calculada', 'pendiente_material', 'lista_para_apartar', 'apartada'], true) ? (float) $material->stock_actual_calculado : (float) $materialDetalle['stock_actual'];
                            $stockReservado = in_array($orden->estado, ['calculada', 'pendiente_material', 'lista_para_apartar', 'apartada'], true) ? (float) $material->stock_reservado_calculado : (float) $materialDetalle['stock_reservado'];
                            $stockDisponible = in_array($orden->estado, ['calculada', 'pendiente_material', 'lista_para_apartar', 'apartada'], true) ? (float) $material->stock_disponible_calculado : (float) $materialDetalle['stock_disponible'];
                            $faltante = in_array($orden->estado, ['calculada', 'pendiente_material', 'lista_para_apartar', 'apartada'], true) ? (float) $material->faltante_calculado : (float) $materialDetalle['faltante'];
                        @endphp
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right">{{ number_format((float) ($material->cantidad_apartada ?? 0), 3) }}</td>
                            <td class="px-4 py-3 text-right">{{ number_format((float) ($material->cantidad_consumida ?? 0), 3) }}</td>
                            <td class="px-4 py-3 text-right">
                                @if(in_array($orden->estado, ['borrador', 'calculada', 'pendiente_material', 'lista_para_apartar'], true))
                    ...
                    @empty
                        <tr>
                            <td colspan="14" class="px-4 py-8 text-center text-gray-500">Esta orden no tiene materiales snapshot.</td>
                        </tr>
                    @endforelse
                </tbody>
                <tfoot class="bg-gray-50 text-sm font-semibold text-gray-900">
                    <tr>
                        <td colspan="13" class="px-4 py-3 text-right">Costo estimado materiales</td>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        @if(in_array($orden->estado, ['borrador', 'calculada', 'pendiente_material', 'lista_para_apartar'], true))
            <form method="POST" action="{{ route('huentitan.ordenes-fabricacion.calcular', $orden) }}">
                @csrf
                <button type="submit" class="w-full px-4 py-2 rounded-md bg-[#0B265A] text-white text-sm font-medium hover:opacity-90">Revisar disponibilidad</button>
            </form>
        @else
            <button type="button" class="px-4 py-2 rounded-md border bg-gray-100 text-gray-500 text-sm font-medium cursor-not-allowed" disabled>Revisar disponibilidad</button>
        @endif
        ...
        @else
            <button type="button" class="px-4 py-2 rounded-md border bg-gray-100 text-gray-500 text-sm font-medium cursor-not-allowed" disabled>Sin materiales para compra</button>
        @endif
        @if($orden->estado === 'lista_para_apartar' && ($resumenMateriales['con_faltante'] ?? 0) === 0)
            <form method="POST" action="{{ route('huentitan.ordenes-fabricacion.apartar', $orden) }}" onsubmit="return confirm('Confirmas apartar el material requerido para esta orden de fabricacion?')">
                @csrf
                <button type="submit" class="w-full px-4 py-2 rounded-md bg-green-600 text-white text-sm font-medium hover:bg-green-700">Apartar material</button>
            </form>
        @elseif($orden->estado === 'apartada')
            <button type="button" class="px-4 py-2 rounded-md border border-green-200 bg-green-50 text-green-700 text-sm font-medium cursor-not-allowed" disabled>Material apartado</button>
        @else
        ...
        @if($orden->estado === 'apartada')
            <form method="POST" action="{{ route('huentitan.ordenes-fabricacion.enviar-produccion', $orden) }}" onsubmit="return confirm('Confirmas enviar esta orden a produccion? Se descontaran los insumos apartados del inventario.')">
                @csrf
                <button type="submit" class="w-full px-4 py-2 rounded-md bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Enviar a produccion</button>
            </form>
        @elseif($orden->estado === 'en_produccion')
            <button type="button" class="px-4 py-2 rounded-md border border-indigo-200 bg-indigo-50 text-indigo-700 text-sm font-medium cursor-not-allowed" disabled>En produccion</button>
        @else
            <button type="button" class="px-4 py-2 rounded-md border bg-gray-100 text-gray-500 text-sm font-medium cursor-not-allowed" disabled>Enviar a produccion</button>
        @endif
