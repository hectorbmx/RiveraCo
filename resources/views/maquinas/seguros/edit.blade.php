...
@section('title', 'Editar seguro de máquina')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">
                Máquina: {{ $maquina->codigo ?? $maquina->nombre ?? ('#' . $maquina->id) }}
            </p>
        </div>
    ...
    <form
        action="{{ route('maquinas.seguros.update', [$maquina, $seguro]) }}"
        method="POST"
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input
                    ...
                    value="{{ old('frecuencia_pago', $seguro->frecuencia_pago) }}"
                    class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500"
            ...
            <div>
                <label class="inline-flex items-center gap-2 mt-6">
                    <input
                        type="checkbox"
                        name="alerta_vencimiento_activa"
                        value="1"
                        {{ old('alerta_vencimiento_activa', $seguro->alerta_vencimiento_activa) ? 'checked' : '' }}
                        class="rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                    >
                    <span class="text-sm text-slate-700">Activar alerta de vencimiento</span>
                </label>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Días de preaviso</label>
                <input
                    type="number"
                    min="1"
                    max="365"
                    name="dias_preaviso"
                    value="{{ old('dias_preaviso', $seguro->dias_preaviso ?? 30) }}"
                    class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500"
                >
            </div>
