        ...
        <nav class="flex flex-wrap gap-2">
            @foreach($tabs as $key => $label)
                ...
                <a href="{{ route('obras.edit', ['obra' => $obra->id, 'tab' => $key]) }}"
                   class="rounded-xl px-4 py-2 text-sm border {{ $isActive ? 'border-[#FFC107] bg-[#0B265A] text-white font-semibold shadow-sm' : 'border-transparent bg-slate-50 text-slate-500 hover:bg-slate-100 hover:text-slate-800' }}">
                    {{ $label }}
                </a>
            @endforeach
        </nav>
            ...
            <tbody class="divide-y divide-slate-100">
                @foreach($facturasSatObra as $factura)
                    <tr>
                        ...
                        <td class="px-3 py-2 text-center">
                            <span class="inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $factura['estado_badge_class'] ?? 'bg-slate-100 text-slate-700 border border-slate-200' }}">
                                {{ $factura['estado_label'] ?? 'Relacionada' }}
                            </span>
                        </td>
                        <td class="px-3 py-2 text-right">
    ...
    <div class="relative bg-white w-full max-w-2xl rounded-xl p-6 shadow-xl">
        ...
        <form method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <label class="block text-xs font-semibold text-slate-600 mb-1">Evidencia fisica</label>
                    <input type="file"
                           name="comprobante"
                           accept="application/pdf,image/*"
                           class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-slate-700 hover:file:bg-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
