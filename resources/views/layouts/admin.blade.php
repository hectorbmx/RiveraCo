...
<div x-data="{ openSat: false }" class="w-full">
    ...
    <div x-show="openSat" x-transition class="ml-10">
        ...
        <!-- Empresas -->
          @if($canMenu('sat.empresas.access'))
        ...
        <!-- Solicitudes -->
          @if($canMenu('sat.solicitudes.access'))
        ...
        <!-- CFDIs -->
          @if($canMenu('sat.documentos.access'))
        <a href="{{ route('sat.cfdis.index') }}"
          ...
          @if($canMenu('sat.facturacion.access'))
...
<div
    ...
    x-show="show"
    x-transition
    class="fixed top-5 right-5 z-50 bg-red-600 text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-3"
>
    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
         viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
