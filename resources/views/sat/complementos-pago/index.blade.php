...
@section('content')
<div x-data="complementosPagoEnvio()" class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
    @php
        $tab = $tab ?? 'registrados';
        $tabBaseQuery = request()->except(['tab', 'page']);
        $estadoOptions = $estados->mapWithKeys(fn ($estadoOption) => [$estadoOption => ucfirst($estadoOption)])->all();
        $kpiChips = [
            [
                'label' => 'Complementos este mes',
                'value' => number_format($kpis['timbrados_mes']),
                'class' => 'text-emerald-600',
            ],
            [
                'label' => 'Monto complementado',
                'value' => '$' . number_format($kpis['monto_timbrado_mes'], 2),
                'class' => 'text-slate-900',
            ],
            [
                'label' => 'Facturas PPD pendientes',
                'value' => number_format($kpis['facturas_pendientes']),
                'class' => 'text-amber-600',
            ],
            [
                'label' => 'Monto PPD pendiente',
                'value' => '$' . number_format($kpis['monto_pendiente'], 2),
                'class' => 'text-red-600',
            ],
        ];
    @endphp
    ...
    <x-filters.card action="{{ route('sat.complementos-pago.index') }}" class="mb-3 p-3">
        ...
        <x-filters.actions
            ...
            clear-url="{{ route('sat.complementos-pago.index', ['tab' => $tab]) }}"
            span="md:col-span-2" />
    </x-filters.card>

    <div class="mb-4 flex flex-wrap items-center gap-2">
        @foreach($kpiChips as $chip)
            <div class="inline-flex min-h-10 items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm shadow-sm">
                ...
                <span class="font-bold {{ $chip['class'] }}">{{ $chip['value'] }}</span>
            </div>
        @endforeach
    </div>

    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6">
        <div class="border-b border-slate-200 bg-slate-50 px-4 pt-4">
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('sat.complementos-pago.index', array_merge($tabBaseQuery, ['tab' => 'registrados'])) }}"
                   class="inline-flex items-center gap-2 rounded-t-xl border px-4 py-2 text-sm font-semibold transition {{ $tab === 'registrados' ? 'border-slate-200 border-b-white bg-white text-slate-900' : 'border-transparent text-slate-500 hover:text-slate-800' }}">
                    Complementos registrados
                    <span class="rounded-full {{ $tab === 'registrados' ? 'bg-slate-900 text-white' : 'bg-white text-slate-500' }} px-2 py-0.5 text-xs">
                        {{ $pagos->total() }}
                    </span>
                </a>

                <a href="{{ route('sat.complementos-pago.index', array_merge($tabBaseQuery, ['tab' => 'pendientes'])) }}"
                   class="inline-flex items-center gap-2 rounded-t-xl border px-4 py-2 text-sm font-semibold transition {{ $tab === 'pendientes' ? 'border-slate-200 border-b-white bg-white text-slate-900' : 'border-transparent text-slate-500 hover:text-slate-800' }}">
                    Por complementar
                    <span class="rounded-full {{ $tab === 'pendientes' ? 'bg-amber-500 text-white' : 'bg-white text-slate-500' }} px-2 py-0.5 text-xs">
        ...
        @if($tab === 'pendientes')
            <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-slate-900">Por complementar</h2>
